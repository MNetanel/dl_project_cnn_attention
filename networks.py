...
# ==================================================
#
# Code from https://github.com/maciejbalawejder/Deep-Learning-Collection
#
# ---------------------------------
...
class ConvBlock(nn.Module):
    '''
    A convolution + batch normalization block.
    From the paper: "We adopt batch normalization right after each convolution and before activation"
    '''
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, first=False):
        super().__init__()
        res_channels = in_channels // 4 # 256 -> 64
        stride = 1
        ...
        for i in range(len(out_features)):
            if i > 0:
                self.blocks.append(ResidualBlock(
                    out_features[i-1], out_features[i]))
            for _ in range(no_blocks[i]-1):
                self.blocks.append(ResidualBlock(
                    out_features[i], out_features[i]))

        self.conv1 = ConvBlock(in_channels, 64, 7, 2, 3) # padding = 0.5 * (stride * (output_size - 1) - input_size + kernel_size)
